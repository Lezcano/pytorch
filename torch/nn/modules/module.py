...
class Module(object):
    ...
    def register_parametrization(self, parametrization, tensor_name):
        r"""Adds a parametrization to ``module[tensor_name]``

        When accessing ``module[tensor_name]``, the module will return the
        parametrized version ``param(module[tensor_name])``
        ...
        Args:
            ...
            tensor_name (string): parameter or parametrization to which the
            parametrization will be applied
        """
        if not hasattr(self, tensor_name):
            raise ValueError("Module '{}' does not have a parameter '{}'"
                             .format(self, tensor_name))

        # We could allow to put parametrizations on buffers as well
        orig = self._parameters.get(tensor_name)
        if orig is not None:
            # Putting a parametrization on a Parameter
            # Register the parametrization
            self._parametrizations[tensor_name] = parametrization
            # Move the original parameter to the parametrization module
            ...
            if prev_param is not None:
                # Register the parametrization at the beginning of the parametrization chain
                self._parametrizations[tensor_name] = parametrization
    ...
    def undo_parametrization(self, tensor_name):
        ...
        It moves the original parameter back to the module.
    ...
    def remove_parametrization(self, tensor_name):
        ...
        It leaves the parametrized tensor in the module as a Parameter.

        .. warning ::

            If the parametrizations change the size of the tensor, the parameters
            on the optimizer have to be manually updated via
    ...
    def __getattr__(self, name):
        ...
        if '_parametrizations' in self.__dict__:
            ...
            if name in parametrizations:
                return parametrizations[name].cache
