...
void Slice::add_start_tensor(Tensor tensor) {
  start_tensor_ = tensor;
}

void Slice::add_stop_tensor(Tensor tensor) {
  stop_tensor_ = tensor;
}

void Slice::add_step_tensor(Tensor tensor) {
  step_tensor_ = tensor;
}
...
TensorIndex::TensorIndex(
  ...
  if (slice.size() == 0) {
    ...
    slice_ = unpackSlice(
      ...
      *(slice.begin() + 1),
      c10::nullopt);
    if (slice_tensors.size() > 0) {
      TORCH_INTERNAL_ASSERT(slice_tensors.size() == 2);
      slice_.add_start_tensor(slice_tensors[0]);
      slice_.add_stop_tensor(slice_tensors[1]);
    }
    ...
    slice_ = unpackSlice(
      ...
      *(slice.begin() + 2));
    if (slice_tensors.size() > 0) {
      TORCH_INTERNAL_ASSERT(slice_tensors.size() == 3);
      slice_.add_start_tensor(slice_tensors[0]);
      slice_.add_stop_tensor(slice_tensors[1]);
      slice_.add_step_tensor(slice_tensors[2]);
    }
  } else {
    TORCH_CHECK_VALUE(
      false,
...
Tensor applySelect(const Tensor& self, int64_t dim, const TensorIndex& index, int64_t real_dim=0) {
  TORCH_INTERNAL_ASSERT(index.is_integer());

  if (torch::jit::tracer::isTracing() && index.is_integer_with_tensor()) {
  ...
  TORCH_CHECK_INDEX(
    !(unpacked_index == 0 && dim == 0 && self.dim() == 0),
    "invalid index of a 0-dim tensor. ",
    "Use tensor.item() to convert a 0-dim tensor to a number");

  int64_t size = self.size(dim);
  TORCH_CHECK_INDEX(
    unpacked_index >= -size && unpacked_index < size,
